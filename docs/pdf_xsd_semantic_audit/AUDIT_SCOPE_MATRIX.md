...
Status: active control matrix; refreshed after Base / General historical schema-family closure.
...
|---|---|---|---|---|---|
| Base / General Conventions | 301-2 | 1.0, 2.0, 2.1, 2.2, 2.3, 2.4 | original V1.0 aggregate preserved as `official:VDV-301-1.0`; later releases service-family based | first pass completed | BG-001/BG-002; four same-path official V1.0 revision collisions; local aggregate compile pending. |
...
| JourneyInformationService | 301-2-6 | 1.0 | release-context-specific V1.0 revisions plus later root blob | first pass completed | JIS findings + BG-001 routing rule. |
| NetworkLocationService | 301-2-7 | 1.0 | V1.0 | first pass completed | |
| PassengerCountingService | 301-2-8 | 1.0, 2.1 | release-context-specific V1.0 revision plus V2.1 | first pass completed | PCS findings + BG-001 routing rule. |
| Ticketing / TicketInformation | 301-2-9 | 1.0 | release-context-specific V1.0 revisions | first pass completed | TKT findings + BG-001 routing rule. |
...
| Network infrastructure | 301-3 | 02-2020 | non-service discovery/network context | **next** | DNS-SD, HTTP and network-level constraints for SDK runtime checks. |

## Current priority

```text
docs/pdf_xsd_semantic_audit/22_network_infrastructure_discovery_context.md
```
