...
Status: semantic/provenance first pass and planned XSD executable-evidence phase completed; runtime/protocol validation phase starting.
...
|---|---|---|---|---|---|
| Base / General Conventions | 301-2 | 1.0, 2.0, 2.1, 2.2, 2.3, 2.4 | deduplicated V1.0 service set; legacy aggregate roots mapped in `schema_profiles` | first pass completed + storage refinement | BG-001/BG-002 refined; runtime HTTP/discovery rules enter block 25. |
...
| TimeService | 301-2-10 | 1.0 | non-XSD SNTP/DNS-SD | first pass completed; runtime phase pending | SNTP profile is a block-25 target. |
| VideoLiveService | 301-2-11 | 1.0, 2.0 | V1.0 strict-XSD unresolved; V2.0 official | VLS-002 executable-confirmed | run 33111119723. RTSP/RTP remains runtime layer. |
...
| HTMLDisplayService | 301-2-17 | 2.1, 2.2, 2.2a | non-XSD HTTP/discovery profile | first pass completed; runtime phase pending | HTTP Content-Type/discovery are block-25 targets. |
...
| Network infrastructure | 301-3 | 02-2020 | non-XSD physical/network profile + discovery runtime context | first pass completed; runtime phase pending | discovery/network checks move into block 25. |
| Cross-service subscription modelling | 301-2 conventions + services | historical | generic Common structures + service-specific exceptions + operation-manifest layer | first pass completed | TSD-003 now confirms response-context resolver requirement. |
...
| Runtime/protocol validation matrix | cross-version | HTTP/DNS-SD/SNTP/RTSP/RTP | VDV rules + external standards + diagnostics | **next / block 25** | begin with authority/source matrix and HTTP Content-Type. |
