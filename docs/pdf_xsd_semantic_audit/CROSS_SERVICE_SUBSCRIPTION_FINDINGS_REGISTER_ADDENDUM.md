...
## SUB-001 - General-Conventions `TerminateSubscribe*` types vs Common `Unsubscribe*`
...
State: cross-service modelling observation; technical intent not fully proven.
...
TSD-003 -> remains open service_modelling_or_generic_response_candidate pending local/runtime tests.
```
